			...
			<form action="" id="manage-menu">
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								<label class="control-label">Description du Menu </label>
					...
					<div class="card-footer">
						<div class="row">
							<div class="col-md-12">
								...
								<button class="btn btn-sm btn-default col-sm-3" type="button" onclick="$('#manage-menu').get(0).reset()"> Annuler</button>
							</div>
						</div>
					</div>
				</div>
							...
							<tbody>
								...
								<tr>
									...
									<td class="">
										...
										<p>Prix : <b><?php echo number_format($row['price'],2 ) ?> Fcfa</b></p>
									</td>
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_menu" type="button" data-id="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-status="<?php echo $row['status'] ?>" data-description="<?php echo $row['description'] ?>" data-price="<?php echo $row['price'] ?>" data-img_path="<?php echo $row['img_path'] ?>">Modifier</button>
										<button class="btn btn-sm btn-danger delete_menu" type="button" data-id="<?php echo $row['id'] ?>">Supprimer</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('#manage-menu').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Données ajoutées avec succès",'succès')
					setTimeout(function(){
						location.reload()
					},1500)
					...
					alert_toast("Données mises à jour avec succès",'succès')
					setTimeout(function(){
						location.reload()
					},1500)
	...
	$('.delete_menu').click(function(){
		_conf("Êtes-vous sûr de vouloir supprimer cette catégorie ?","delete_menu",[$(this).attr('data-id')])
	})
